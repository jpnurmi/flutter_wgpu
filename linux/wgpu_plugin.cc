...
#define WGPU_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), wgpu_plugin_get_type(), \
                              WgpuPlugin))
...
static void wgpu_plugin_handle_method_call(
    WgpuPlugin* self,
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  WgpuPlugin* plugin = WGPU_PLUGIN(user_data);
  wgpu_plugin_handle_method_call(plugin, method_call);
}

void wgpu_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  WgpuPlugin* plugin = WGPU_PLUGIN(
      g_object_new(wgpu_plugin_get_type(), nullptr));
